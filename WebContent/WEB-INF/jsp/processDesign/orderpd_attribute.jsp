...
<input type="hidden" name="flow_id" value="${node.flowId }"/>
<input type="hidden" name="process_id" value="${node.gid }"/>
  <div class="tab-content">
    <div class="tab-pane active" id="attrBasic">
          ...
          <div class="control-group">
            ...
            <div class="controls">
              <input type="text" id="process_name" placeholder="步骤名称" name="node.nodeName" value="${node.nodeName }">
            </div>
          </div>
        ...
        <div class="control-group">
            ...
            <div class="controls">
              <select name="auto_person" id="auto_person_id">
                <!-- <option value="0">不自动选人</option>    0：无  1:指定人 2：角色 3：组
                <option value="3" >发起人</option>
                <option value="4" >发起人的部门主管</option>
                <option value="5" >处理人的部门主管</option> -->
                <option value="2" selected="selected">指定角色</option>
            ...
            <div id="auto_person_5" >
              <div class="control-group">
                ...
                <div class="controls">
                    <input type="hidden" name="auto_role_ids" id="auto_role_value" <c:if test="${node.doType==2 }">value="${node.forIds }"</c:if> >
                    <input class="input-xlarge" readonly="readonly" type="text" placeholder="指定角色" name="auto_role_text" id="auto_role_text" <c:if test="${node.doType==2 }">value="${roleNames }"</c:if>> 
                    <a href="javascript:void(0);" class="btn" onclick="superDialog('${ctx}/flow/select_roleSelect.emi?selectValue='+document.getElementById('auto_role_value').value,'auto_role_text','auto_role_value');">选择</a>
                </div>
                <label class="control-label">智能发送</label>
                <div class="controls">
                	<input type="checkbox" name="smartSendType" id="smartSendType_1" value="1" <c:if test="${smartType_1_select==1 }">checked="checked"</c:if>>根据数据权限判定发送人
                	<br/>
                	<input type="checkbox" name="smartSendType" id="smartSendType_2" value="2" <c:if test="${smartType_2_select==1 }">checked="checked"</c:if>>只有一个可发送人时自动发送
                </div>
              </div>
            </div>
...
<hr/>
		<h4>数据操作</h4>
		<div class="control-group" >
            <label class="control-label">表单编辑</label>
            <div class="controls">
               <input type="checkbox" name="node.allowEdit" id="allowEdit" value="1" <c:if test="${node.allowEdit==1 }">checked="checked"</c:if>>允许编辑
            </div> 
          </div>
     <!-- <h4>授权范围</h4>
     	  <div class="control-group" >
