	...
	$(function() {
		...
		$('.addBtn').click(function() {
			var pwdWin = $.dialog({ 
			    ...
			    width: '500px',
				height: '150px',
				content: 'url:${ctx}/wms/prty_getAA_BaseAttrAdd.emi',
				okVal:"保存",
				ok:function(){
					...
					$.ajax({
		 				  ...
		 				  type: 'POST',
		 				  url: '${ctx}/wms/prty_AddAA_BaseAttr.emi',
		 				 dataType:'text',
		 				  success: function(req){
		 					  if(req=='success'){
		 						  $.dialog.alert_s('保存成功',function(){location.href="${ctx}/wms/prty_getAA_BaseAttrList.emi";});
		 					  }
		 					  else if(req=='fail'){
		...
		$('.editBtn').click(function() {
			...
			var pwdWin = $.dialog({ 
			    ...
			    width: '500px',
				height: '150px',
				content: 'url:${ctx}/wms/prty_getAA_BaseAttrAdd.emi?gid='+gid,
				okVal:"保存",
				ok:function(){
				 	if(!this.content.checkdata()){
						return false;
					...
					$.ajax({
		 				  data: $("#myform",this.content.document).serialize(),
		 				  type: 'POST',
		 				  url: '${ctx}/wms/prty_AddAA_BaseAttr.action',
		 				 dataType:'text',
		 				  success: function(req){
		 					  if(req=='success'){
		 						  $.dialog.alert_s('保存成功',function(){location.href="${ctx}/wms/prty_getAA_BaseAttrList.emi";});
		...
		$('.delBtn').click(function() {
			...
			$.dialog.confirm(tip_confirmDelete, function(){		
				...
				$.ajax({
					  ...
					  type: 'POST',
					  url: '${ctx}/wms/prty_DeleteAA_BaseAttr.emi',
					  dataType:'text',
					  success: function(req){
						  if(req=='success'){
...
function serch(gid)
{
	location.href="${ctx}/wms/prty_getAA_BaseAttrValList.emi?gid="+gid;
}
</script>
</head>
			...
			<!--按钮部分-->
		 	<div class="toolbar">
		 		<ul>
		 			...
		 			<li class="fl"><input type="button" id="AttributeProjectsearch" class="searchBtn" value="查询"> </li>
		 			<div class="cf"></div>
		 		</ul>
		 	</div>
		 	<!--按钮部分 end-->
		 			...
		 			<table>
						...
						<c:forEach var="bean" items="${data.list }" varStatus="stat">	 
			 				<tr>
								...
								<td>${bean.name}</td>
			 					<td><a href="#" onclick="serch('${bean.gid }');">查看属性值</a></td>
			 				</tr>
	 				 	</c:forEach>
			 			</tbody>
			 		</table>
