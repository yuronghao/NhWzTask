...
public class SynchroDaoT6 extends BaseDao {
	...
	public int addInventory(){
		String sql="insert into AA_Goods(gid,goodsCode,goodsName,goodsStandard,unitForSynchro,sortGidForSynchro,bInvBach,defWareHouseForSynchro,cassComUnitForSynchro,goodsBarCode,cstComUnitForSynchro) "+
				"SELECT newid(),cInvCode,cInvName,cInvStd,cComUnitCode,cInvCCode,bInvBatch,cDefWareHouse,cAssComUnitCode,cInvCode,cSTComUnitCode from "+Config.BUSINESSDATABASE+"Inventory "+
	...
	public int updInventory(){
		String sql="update AA_Goods set goodsName=cInvName,goodsStandard=cInvStd,unitForSynchro=cComUnitCode,sortGidForSynchro=cInvCCode,"
				...
				+ " isnull(gs.cstComUnitForSynchro,'')<>it.cSTComUnitCode  ";
	...
	public int addWareHouse(){
		String sql="insert into AA_WareHouse(gid,whCode,whName,whPos) select newid(),cWhCode,cWhName,bWhPos FROM "+Config.BUSINESSDATABASE+"Warehouse "+
	...
	public int updWareHouse(){
		...
		String sql="update AA_WareHouse set whName=w.cWhName,whPos=w.bWhPos from AA_WareHouse aw "+
					"left join "+Config.BUSINESSDATABASE+"Warehouse w on aw.whCode=w.cWhCode "+
					"where isnull(aw.whName,'')<>w.cWhName OR "+
					"isnull(aw.whPos,'')<>w.bWhPos";
		return this.update(sql);
	}
	 ...
	 */
	public int addUser(){
		String sql="insert into YM_User(gid,userCode,userName)  select newid(),cUser_Id,cUser_Name  FROM UFSystem.dbo.UA_User "+
					"where cUser_Id not in (select userCode from YM_User) ";
		return this.update(sql);
	}
	...
	public int updUser(){
		String sql="update YM_User set userName=u.cUser_Name from YM_User yu "+
					"left join UFSystem.dbo.UA_User u on yu.userCode=u.cUser_Id "+
	...
	public int deletePerson(){
		String sql="delete from AA_Person where perCode not in (select cPersonCode from "+Config.BUSINESSDATABASE+"Person ) ";
		return this.update(sql);
	}
	...
	public int addPerson(){
		String sql="insert into AA_Person(gid,perCode,perName,depCodeForSynchro)  select newid(),cPersonCode,cPersonName,cDepCode FROM "+Config.BUSINESSDATABASE+"Person "+
					"where  cPersonCode not in (select perCode from AA_Person)";
		return this.update(sql);
	}
	...
	public int updPerson(){
		String sql="update AA_Person set perName=p.cPersonName,depCodeForSynchro=p.cDepCode,barcode='P'+ap.perCode from AA_Person ap "+
					"left join "+Config.BUSINESSDATABASE+"Person p on ap.perCode=p.cPersonCode "+
					"where isnull(ap.perName,'')<>p.cPersonName OR "+
					"isnull(ap.depCodeForSynchro,'')<>p.cDepCode or "
	...
	public int addProArrivalC(){
		String sql="insert into WM_ProcureArrival_C(gid,autoidForSynchro,goodsCodeForSynchro,number,assistNumber,unitPrice,totalPrice,needCheck,"
				...
				+ "putinNumber,idForSynchro,batch,putinAssistNumber,define22,posIDForSynchro,cfree1,cfree2) select newid(),autoid,cInvCode,iQuantity,iNum,iCost,iMoney,bGsp,"
				+ "iOriTaxCost,iOriSum,iOriCost,iOriMoney,iOriTaxPrice,"
				+ "iOriTaxCost,iSum,iCost,iMoney,iTaxPrice,"
				+ "fValidInQuan,id,cbatch,fValidInNum,cdefine22,iPOsID,cfree1,cfree2  from "+Config.BUSINESSDATABASE+"PU_ArrivalVouchs  "
	...
	public int updProArrivalC(){
		...
		String sql="update WM_ProcureArrival_C set goodsCodeForSynchro=cInvCode,number=iQuantity,assistNumber=iNum,unitPrice=iCost,totalPrice=iMoney,needCheck=bGsp, "+
				...
				+ "putinNumber=fValidInQuan,batch=cbatch,putinAssistNumber=fValidInNum,define22=cdefine22,posIDForSynchro=iPOsID,cfree1=paus8.cfree1,cfree2=paus8.cfree2 "
				...
				+ " isnull(pac.cfree2,'')<>isnull(paus8.cfree2,'') ";
	...
	public int deleteSaleSend(){
		String sql="delete from WM_SaleSend where billCode not in (select cDLCode from "+Config.BUSINESSDATABASE+"DispatchList )";
		return this.update(sql);
	}
	...
	public int addSaleSend(){
		String sql="insert into WM_SaleSend(gid,customerCodeForSynchro,rate,billCode,billState,billDate,isReturn,autoidForSynchro,define1,stateForSynchro) "+
					...
					"where cDLCode not in (select billCode from WM_SaleSend) ";
		return this.update(sql);
	}
	...
	public int updSaleSend(){
		String sql="update WM_SaleSend set customerCodeForSynchro=cCusCode,rate=iTaxRate,billState=iverifystate,billDate=dDate,isReturn=bReturnFlag,define1=cDefine1,stateForSynchro=cVerifier "+
					"from WM_SaleSend ss LEFT JOIN "+Config.BUSINESSDATABASE+"DispatchList dis on ss.billCode=dis.cDLCode "+
	...
	public int deleteProduceOrder(){
		String sql="delete from WM_ProduceOrder where isnull(autoidForSynchro,'') not in (select id from "+Config.BUSINESSDATABASE+"PP_ProductPO )";
		return this.update(sql);
	}
	...
	public int addProduceOrder(){
		String sql="INSERT INTO WM_ProduceOrder (gid,billCode,billDate,autoidForSynchro,depCodeForSynchro,managerCodeForSynchro ) "+
					"SELECT newid(),cCode,dDate,id,cDepCode,cPersonCode FROM "+Config.BUSINESSDATABASE+"PP_ProductPO "+
					"WHERE id NOT IN (SELECT autoidForSynchro FROM WM_ProduceOrder) ";
		return this.update(sql);
	}
	...
	public int updProduceOrder(){
		String sql="update WM_ProduceOrder set billCode=cCode,billDate=dDate,depCodeForSynchro=cDepCode,managerCodeForSynchro=cPersonCode "+
					"from WM_ProduceOrder wpo LEFT JOIN "+Config.BUSINESSDATABASE+"PP_ProductPO mo on wpo.autoidForSynchro=mo.id "+
					"where isnull(wpo.billCode,'')<>isnull(mo.cCode,'') or "
					+ " isnull(wpo.billDate,'')<>isnull(mo.dDate,'') or "
					+ " isnull(wpo.depCodeForSynchro,'')<> isnull(mo.cDepCode,'') or "
					+ " isnull(wpo.managerCodeForSynchro,'')<> isnull(mo.cPersonCode,'') ";
	...
	/**
	 * @category 修改生产订单主表相关属性
	 *2016 2016年3月24日上午11:00:21
	 *int
	 *宋银海
	 */
	public int updProduceOrderInfor(){
		String sql="UPDATE WM_ProduceOrder "+
					"SET deptGid = d.gid, managerGid=ps.gid "+
					"FROM WM_ProduceOrder p "+
					"LEFT JOIN AA_Department d ON p.depCodeForSynchro=d.depcode "+
					"left join AA_Person ps on p.managerCodeForSynchro=ps.percode "+
					"WHERE  isnull(p.deptGid,'')<>isnull(d.gid,'') or "
					+ " isnull(p.managerGid,'')<>isnull(ps.gid,'') ";
	...
	public int deleteProduceOrderC(){
		String sql="delete from WM_ProduceOrder_C where isnull(autoidForSynchro,'') not in (select MainId from "+Config.BUSINESSDATABASE+"PP_POMain )";
		return this.update(sql);
	}
	...
	public int addProduceOrderC(){
		...
		String sql="insert into WM_ProduceOrder_C(gid,autoidForSynchro,idForSynchro,goodsCodeForSynchro,number,completedNum,startDate,endDate,isCheck) "+
					"select newid(),MainId,id,cInvCode,fQuantity,fInQuantity,dStartDate,dEndDate,bCheck from "+Config.BUSINESSDATABASE+"PP_POMain od "+
					"where od.MainId not in (select autoidForSynchro from WM_ProduceOrder_C) ";
		return this.update(sql);
	}
	...
	public int updProduceOrderC(){
		String sql="update WM_ProduceOrder_C set goodsCodeForSynchro=cInvCode,number=fQuantity,completedNum=fInQuantity,isCheck=bCheck,startDate=dStartDate,endDate=dEndDate,"
				+ " idForSynchro=od.id "+
					"from WM_ProduceOrder_C wpoc LEFT JOIN "+Config.BUSINESSDATABASE+"PP_POMain od on wpoc.autoidForSynchro=od.MainId "+
					"where isnull(wpoc.goodsCodeForSynchro,'')<>isnull(od.cInvCode,'')  or "+
					"isnull(wpoc.number,0)<>isnull(od.fQuantity,0) or "+
					"isnull(wpoc.idForSynchro,'')<>isnull(od.id,'') or "+
					"isnull(wpoc.completedNum,0)<>isnull(od.fInQuantity,0) or "+
					"isnull(wpoc.isCheck,'')<>isnull(od.bCheck,'') or "+
					"isnull(wpoc.startDate,'')<>isnull(od.dStartDate,'') or "+
					"isnull(wpoc.endDate,'')<>isnull(od.dEndDate,'') ";
		return this.update(sql);
	}
	...
	public int updProduceOrderCInfor(){
		String sql="UPDATE WM_ProduceOrder_C "+
					"SET goodsUid = gs.gid, produceOrderUid=po.gid "+
	...
	public int deleteProduceOrderC2(){
		String sql="delete from WM_ProduceOrder_C2 where isnull(autoidForSynchro,'') not in (select subid from "+Config.BUSINESSDATABASE+"PP_PODetails )";
		return this.update(sql);
	}
	...
	public int addProduceOrderC2(){
		...
		String sql="insert into WM_ProduceOrder_C2(gid,autoidForSynchro,goodsCodeForSynchro,number,receivedNum,idForSynchro) "+
					"select newid(),subid,cInvCode,fQuantity,fOutQuantity,MainID  from "+Config.BUSINESSDATABASE+"PP_PODetails "+ 
					"where subid not in (select autoidForSynchro from WM_ProduceOrder_C2) ";
		return this.update(sql);
	}
	...
	public int updProduceOrderC2(){
		String sql="update WM_ProduceOrder_C2 set goodsCodeForSynchro=cInvCode,number=fQuantity,receivedNum=fOutQuantity,idForSynchro=mainId "+
					"from WM_ProduceOrder_C2 wpoc2 LEFT JOIN "+Config.BUSINESSDATABASE+"PP_PODetails mol on wpoc2.autoidForSynchro=mol.subid "+
					"where isnull(wpoc2.goodsCodeForSynchro,'')<>isnull(mol.cInvCode,'') or "+
					"isnull(wpoc2.number,0)<>isnull(mol.fQuantity,0) or "+
					"isnull(wpoc2.receivedNum,0)<>isnull(mol.fOutQuantity,0) or "
					+ " isnull(wpoc2.idForSynchro,0)<>isnull(mol.mainId,0) ";
		return this.update(sql);
	}
	...
	public int updProduceOrderC2Infor(){
		String sql="UPDATE WM_ProduceOrder_C2  "+
				...
				" WHERE  isnull(pc2.goodsUid,'')<>isnull(gs.gid,'') "+
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////发货单子表结束	
	...
	public List<WmAllocationstock> getWasYonYou(String condition){
		String sql="select cWhCode,cInvCode,cBatch,iQuantity,iNum,cFree1,cFree2 from "+Config.BUSINESSDATABASE+"CurrentStock where "+condition;
		return this.getJdbcTemplate().query(sql, new RowMapper(){
			...
			public Object mapRow(ResultSet rs, int arg1) throws SQLException {
				...
				was.setBatch(CommonUtil.Obj2String(rs.getString("cBatch")));
	...
	public List<WmAllocationstock> getWasYonYouPos(String condition){
		String sql="SELECT cPosCode,cWhCode,cInvCode,sum(iQuantity) iQuantity ,sum(iNum) iNum,cBatch,cFree1, cFree2 FROM "+
					"(select cPosCode,cWhCode,cInvCode,sum(iQuantity) iQuantity ,sum(iNum) iNum,isnull(cBatch,'') cBatch,isnull(cFree1,'') cFree1, isnull(cFree2,'') cFree2,bRdFlag "+
					"from "+Config.BUSINESSDATABASE+"InvPosition WHERE bRdFlag=1 group by cPosCode,cWhCode,cInvCode,cBatch,cFree1,cFree2,bRdFlag "+
					"union ALL "+
					 "select cPosCode,cWhCode,cInvCode,-sum(iQuantity) iQuantity,sum(iNum) iNum,isnull(cBatch,'') cBatch,isnull(cFree1,'') cFree1,isnull(cFree2,'') cFree2,bRdFlag "+
					 "from "+Config.BUSINESSDATABASE+"InvPosition WHERE bRdFlag=0 group by cPosCode,cWhCode,cInvCode,cBatch,cFree1,cFree2,bRdFlag ) ps "+
					 "GROUP BY ps.cPosCode,ps.cWhCode,ps.cInvCode,ps.cBatch,ps.cFree1,ps.cFree2  ";
