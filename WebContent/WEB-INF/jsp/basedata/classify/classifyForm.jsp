...
<script type="text/javascript">
	...
	$(function() {
		...
		if("${isRoot}"=="true"){
			document.getElementById("classificationname").disabled=true;
		...
		if(node.length>0){
			if(checkdata()){
				$.ajax({
					  ...
					  type: 'POST',
					  url: '${ctx}/wms/classify_updateClassify.emi',
	...
	function addChild(){
		...
		var node = treeWin.selectedNode();
		if(node.length>0){
			location.href="${ctx}/wms/classify_toAddClassify.emi?parentid=${classify.gid}&stylegid=${classify.stylegid}&isRoot=${isRoot}";
		}else{
			$.dialog.alert_e("请先选择节点");
		}
	}
	...
	<form action="${ctx }/right_updateRight.emi" method="post" id="myform" class="myform"  onsubmit="return checkdata()">
		...
		<div class="toolbar" style="margin-top: 15px">
	 		<ul>
						...
						<input class="delBtn" type="button" value="删除" onclick="deleteClassify()"/>
						&nbsp;&nbsp;
						<input class="addBtn" type="button" value="添加子类别" onclick="addChild()"/>
	 			<div class="cf"></div>
	 		</ul>
	 	</div>
	 	<!--主体部分-->
	 	<div class="mainword">
	 		...
	 		<input type="hidden" name="isRoot" id="isRoot" value="${isRoot }">
	 		<input type="hidden" name="classify.stylegid" id="stylegid" value="${classify.stylegid }">
	 			...
	 			<ul class="EMtree_word fl" style="width: 100%;">
	 				...
	 				<li>
	 					...
	 					<div class="fl mainaddword_word"><input class="addwordinput " type="text" readonly="readonly" name="classify.parentName" id="parentName" value="${f_classify.classificationname }">
	 							<input type="hidden"  name="classify.parentid" id="parentid" value="${classify.parentid }">
	 					 </div>
	 					<div class="cf"></div>
	 				</li>
